...
function getDrawingAngles(state) {
  ...
  */
	...
	const anglePerStage = __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0__utility__["j" /* round */])(maxAngleModification / state.maxAnimationStage * state.animationStage, 2);
...
const Pacman = (canvas, x, y, index, tileWidth, tileHeight) => {
   ...
   */
		updateAnimation() {
			...
			state.animationStage = state.isOpening ? state.animationStage += 1 : state.animationStage -= 1;
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_babel_polyfill__ = __webpack_require__(86);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_babel_polyfill___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0_babel_polyfill__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__Scripts_Game_js__ = __webpack_require__(118);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__utility__ = __webpack_require__(48);
...
function getPossiblePaths(element, map, directions) {
	...
	let possibleDirections = directions.filter(el => el !== __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__utility__["a" /* oppositeDirection */])(element.state.direction));
...
function releaseGhosts(score, maxScore, ghosts) {
	if (score >= maxScore / 8 && __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__utility__["b" /* count */])(ghosts, ghost => ghost.state.frozen) === 2 || score >= maxScore / 5 && __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__utility__["b" /* count */])(ghosts, ghost => ghost.state.frozen) === 1) {
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__Pacman__ = __webpack_require__(87);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__utility__ = __webpack_require__(48);
...
const Ghost = (canvas, x, y, index, tileWidth, tileHeight, color) => {
    ...
    */
			...
			if (scared) {
				...
				ctx.bezierCurveTo(x - width / 2 + width / 1.64, y - height / 2 + height / 3.5, x - width / 2 + width / 1.75, y - height / 2 + height / 2.54, x - width / 2 + width / 1.75, y - height / 2 + height / 2.15);
				...
				switch (direction) {
					case 'left':
						ctx.beginPath();
						ctx.arc(x - width / 2 + width / 1.55, y - height / 2 + height / 2, width / 14, 0, Math.PI * 2, true);
						ctx.fill();

						ctx.beginPath();
						ctx.arc(x - width / 2 + width / 4.66, y - height / 2 + height / 2, width / 14, 0, Math.PI * 2, true);
						ctx.fill();
						break;

					case 'right':
						ctx.beginPath();
						ctx.arc(x - width / 2 + width / 1.25, y - height / 2 + height / 2, width / 14, 0, Math.PI * 2, true);
						ctx.fill();

						ctx.beginPath();
						ctx.arc(x - width / 2 + width / 2.8, y - height / 2 + height / 2, width / 14, 0, Math.PI * 2, true);
						ctx.fill();
						break;

					case 'up':
						ctx.beginPath();
						ctx.arc(x - width / 2 + width / 1.4, y - height / 2 + height / 2.7, width / 14, 0, Math.PI * 2, true);
						ctx.fill();

						ctx.beginPath();
						ctx.arc(x - width / 2 + width / 3.3, y - height / 2 + height / 2.7, width / 14, 0, Math.PI * 2, true);
						ctx.fill();
						break;

					case 'down':
						ctx.beginPath();
						ctx.arc(x - width / 2 + width / 1.4, y - height / 2 + height / 1.8, width / 14, 0, Math.PI * 2, true);
						ctx.fill();

						ctx.beginPath();
						ctx.arc(x - width / 2 + width / 3.3, y - height / 2 + height / 1.8, width / 14, 0, Math.PI * 2, true);
						ctx.fill();
						break;
					default:
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__utility__ = __webpack_require__(48);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__Pacman__ = __webpack_require__(87);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__Ghost__ = __webpack_require__(119);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__levelEditor__ = __webpack_require__(121);
...
const Map = (backgroundCanvas, foregroundCanvas, wallColor) => {
	const state = {
		...
		tileWidth: backgroundCanvas.width / __WEBPACK_IMPORTED_MODULE_3__levelEditor__["a" /* default */].numberOfHorizontalTiles,
	...
	const map = state.layout.map((element, index) => {
		...
		const dinamic = [];
		if (element === 'C') {
			dinamic.push(__webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__Pacman__["a" /* Pacman */])(foregroundCanvas, x, y, index, state.tileWidth, state.tileHeight));
		} else if (element === 'M') {
			dinamic.push(__webpack_require__.i(__WEBPACK_IMPORTED_MODULE_2__Ghost__["a" /* default */])(foregroundCanvas, x, y, index, state.tileWidth, state.tileHeight, ghostColors[0]));
			ghostColors.shift();
		}
   ...
   */
     ...
     */
				if (el.static.type === '#') {
					...
					if (bottomleft) {
						ctx.clearRect(el.static.x - state.tileWidth / 2, el.static.y + state.tileHeight / 2 - wallShrink, wallShrink, wallShrink);
					}
					if (topright) {
						ctx.clearRect(el.static.x + state.tileWidth / 2 - wallShrink, el.static.y - state.tileHeight / 2, wallShrink, wallShrink);
					}
					if (bottomright) {
						ctx.clearRect(el.static.x + state.tileWidth / 2 - wallShrink, el.static.y + state.tileHeight / 2 - wallShrink, wallShrink, wallShrink);
					}
				}
				...
				if (el.static.type === '*') {
					...
					ctx.arc(el.static.x, el.static.y, state.tileWidth / 13, 0, 2 * Math.PI);
				...
				if (el.static.type === '@') {
					...
					ctx.arc(el.static.x, el.static.y, state.tileWidth / 5, 0, 2 * Math.PI);
		...
		initAnimatedElement(index, type, color) {
			...
			if (type === 'C') {
				const pacman = __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1__Pacman__["a" /* Pacman */])(foregroundCanvas, x, y, index, state.tileWidth, state.tileHeight);
				...
				const ghost = __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_2__Ghost__["a" /* default */])(foregroundCanvas, x, y, index, state.tileWidth, state.tileHeight, color);
